...
new Float:fSpeedNative[MAX_PLAYERS+1],Float:fGravityNative[MAX_PLAYERS+1];
public plugin_init()
{
	register_plugin("[ZP] Class: Zombie", fmt("%s.1DusT",ZP_VERSION_STRING), "ZP Dev Team")
...
public plugin_natives()
{
	...
	register_native( "zpv_zombie_speed_set", "_set_speed" );
	register_native( "zpv_zombie_speed_reset", "_reset_speed" );
	register_native( "zpv_zombie_speed_get", "_get_speed" );

	register_native( "zpv_zombie_gravity_set", "_set_gravity" );
	register_native( "zpv_zombie_gravity_reset", "_reset_gravity" );
	register_native( "zpv_zombie_gravity_get", "_get_gravity" );
...
public _set_speed( pl, argc )
{
	new id = get_param( 1 );
	fSpeedNative[ id ] = get_param_f( 2 );

	if( zp_core_is_zombie(id) )
		cs_set_player_maxspeed_auto(id, (Float:ArrayGetCell(g_ZombieClassSpeed, g_ZombieClass[id])+fSpeedNative[id]) )
}
	
public Float:_get_speed( pl, argc )
	return fSpeedNative[ get_param( 1 ) ];
public _reset_speed( pl, argc )
	fSpeedNative[ get_param( 1 ) ] = 0.0;

public _set_gravity( pl, argc )
{
	new id = get_param( 1 );
	fGravityNative[ id ] = get_param_f( 2 );
	if( zp_core_is_zombie( id ) )
		set_user_gravity(id, (Float:ArrayGetCell(g_ZombieClassGravity, g_ZombieClass[id])+fGravityNative[id]) )
}
public Float:_get_gravity( pl, argc )
	return fGravityNative[ get_param( 1 ) ];
public _reset_gravity( pl, argc )
	fGravityNative[ get_param( 1 ) ] = 0.0;
